...
public:
	...
	int   m_OldMode;
	CApplication() :
		m_Time(0.0f), m_SolveTime(0), m_IterationNum(20), m_Mode(eModeXPBD_Wood), m_OldMode(eModeMax) {}
...
public:
	CConstraint(CParticle* p0, CParticle* p1) :
		m_RestLength(0.0f), // 静态长度
		m_Particle1(p0), // 粒子1
		m_Particle2(p1), // 粒子1
		m_Stiffness(0.1f), // 刚度
		m_Compliance(0.0f), // 柔度
		m_Lambda(0.0f)  // lambda
	{ 
		glm::vec3 p0_to_p1 = m_Particle2->GetPosition() - m_Particle1->GetPosition();
		m_RestLength = glm::length(p0_to_p1); // 更新静态长度
	}
	...
	void Solve(CApplication& app, float dt) {
		...
		if (app.m_Mode != eModePBD) { // XPBD
			m_Compliance = MODE_COMPLIANCE[app.m_Mode]; // 查看布料的柔度系数
...
void idle(void) {
	...
	g_Ball.Update(dt); // 更新相关选项
	g_Cloth.Update(g_Application, dt, &g_Ball, g_Application.m_IterationNum); // 更新相关选项

	g_Application.SetTime(time); 
	glutPostRedisplay(); // 重新把他俩画出来
}
...
int main(int argc, char* argv[]) {
	...
	glutDisplayFunc(display); // 窗口重新绘制的时候调用
	glutReshapeFunc(reshape); // reshape会重新更新投影矩阵
	glutIdleFunc(idle);
	glutKeyboardFunc(keyboard); // 键盘回调函数
	glutSpecialFunc(special); // 特殊字符回调函数

	glutMainLoop(); // 进入main循环，如果用户没有任何反馈，则进入idle，idle绘Update各种变量，之后调用display
	return 0;
}
